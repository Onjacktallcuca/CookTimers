...
export default function Timer({ name, duration, originalDuration }: TimerProps) {
  ...
  const alarmSound = useRef(new Audio("../src/assets/alarme.mp3")); // Adicione o caminho para o seu arquivo de som

  if (remainingTime <= 0 && interval.current) {
    clearInterval(interval.current);
    alarmSound.current.play(); // Toca o som quando o timer atinge zero
  }
